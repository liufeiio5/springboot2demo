...
public class DailyRecordController {
    ...
    public R addDaily(HttpSession session, String typeName, Integer type, Integer surface, Integer line,
        ...
        if (surfaceName != null && !"".equals(surfaceName)) {
            //判断当前新增面是否重复
            List<Surface> addSurfaceList = surfaceService.findAllSurFace(addSurface.setSurfaceName(surfaceName).
        ...
        if (lineName != null && !"".equals(lineName)) {
            //判断当前新增线是否重复
            List<Line> addLineList = lineService.findLineAll(addLine.setLineName(lineName).
                    setCreateUser(user.getId()));
        ...
        if (pointName != null && !"".equals(pointName)) {
            //判断当前新增点是否重复
            List<Point> addPointList = pointService.findPointAll(addPoint.setPointName(pointName).
                    setCreateUser(user.getId()));
