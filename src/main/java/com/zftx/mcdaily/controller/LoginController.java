...
public class LoginController {
    ...
    @ResponseBody
    public R getDailyInfo(DailyRecord dailyRecord,HttpSession session){
        //获取当前登录用户的信息
        User user = (User) session.getAttribute("user");
        //设置查询条件为当前登录用户的ID
        //event.setCreateUser(user.getId());
        //List<HashMap<String,Object>> eventList = eventService.findEventByEventDetail(event,eventDetail);
        List<DailyRecord> dailyRecords = dailyRecordService.getDailyRecord(dailyRecord.setUserId(user.getId()));
        return R.ok().put("data",dailyRecords).put("username",user.getUserName());
    }
